...
> ⚠️ **Superseded (2026-08-04), partially.** This ADR was written checking against the **HustlerPay** backend ("no object named License exists... across HustlerPay's `src/`") — before the **MCP backend** (`momoassistant-platform`) existed. RFC-0010 (Customer Platform Domain Foundation, Accepted) and CP-001.1/CP-001.2 (shipped) since built the real `License`/`Subscription`/`Invoice` model in MCP, and it does **not** match this document's "Technical License" definition below (§ *Technical License — exact role*): MCP's `License` is **Organization-scoped** (a commercial/billing gate tied to a `ProductPlan`), not a per-Runtime/per-device authorization credential, and it has no relationship to HustlerPay's Device Token at all. **The historical content below is kept intact for the record — do not treat its "Technical License" definition, or the Reference Architecture Diagram's `Device Token (Identity)` node, as current.** The higher-level chain (`Organization → Subscription → License → Entitlements`) and the public-facing terminology (Subscription, Entitlements, Packaging) still hold and match MCP's real shape closely enough that `/pricing` and `/workspace-access`'s existing copy needed no rewrite (see WS-005, `docs/programs/web-platform/` in the `momoassistant-platform` repo, for the audit that found this). For the current, authoritative model, see RFC-0010 in that repo.
